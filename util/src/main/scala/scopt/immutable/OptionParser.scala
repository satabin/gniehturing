...
abstract case class OptionParser[C](
  ...
  def help(shortopt: String, longopt: String, description: String) =
    new FlagOptionDefinition[C](Some(shortopt), longopt, description, { _ => this.showUsage; sys.exit })

  def help(shortopt: Option[String], longopt: String, description: String) =
    new FlagOptionDefinition[C](shortopt, longopt, description, { _ => this.showUsage; sys.exit })
